...
export default {
  ...
  `;
    ...
    function whereToReturn() {
      ...
      if (document.referrer || localStorage.getItem('full_community_path')) {
        if (document.referrer) {
          localStorage.removeItem('full_community_path');
        }
        text = 'Back to School';
        if(document.referrer && document.referrer.includes('sso')){
    ...
    window.addEventListener('load', () => {
      ...
      const communitySideBarElement = document.querySelector('.header-sidebar-toggle')
      if(!communitySideBarElement) return
      communitySideBarElement.addEventListener('click', function(){
        var locationString = whereToReturn();
        var divEl = document.createElement('div');
        divEl.className = 'link-wrapper';
        divEl.innerHTML = svgIcon(locationString, text)
